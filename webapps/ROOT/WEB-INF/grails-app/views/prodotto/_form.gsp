...
<%--<div class="fieldcontain ${hasErrors(bean: prodottoInstance, field: 'durata', 'error')} ">
	<label for="durata"> <g:message code="prodotto.durata.label"
			default="Durata" />
	</label>
...
<div class="fieldcontain ${hasErrors(bean: prodottoInstance, field: 'commenti', 'error')} ">
	...
	<ul class="one-to-many">
		<g:each in="${prodottoInstance?.commenti?}" var="c">
			<li>
                <g:link controller="commento" action="show" id="${c.id}">
					${c?.encodeAsHTML()}
				</g:link>
			</li>
		</g:each>
		<%--<li class="add">
            <g:link controller="commento" action="create" params="['prodotto.id': prodottoInstance?.id]">
				${message(code: 'default.add.label', args: [message(code: 'commento.label', default: 'Commento')])}
            </g:link>
        </li>--%>
	</ul>
</div>
...
<div class="fieldcontain ${hasErrors(bean: prodottoInstance, field: 'linea', 'error')} required">
	...
	<g:select id="linea" name="linea.id"
		from="${it.ggg.sa.prodotto.Linea.list()}" optionKey="id" required=""
		value="${prodottoInstance?.linea?.id}" class="many-to-one" />
</div>
...
<div class="fieldcontain ${hasErrors(bean: prodottoInstance, field: 'tipoProdotto', 'error')} required">
	...
	<g:select id="tipoProdotto" name="tipoProdotto.id"
...
<%--<div class="fieldcontain ${hasErrors(bean: prodottoInstance, field: 'top', 'error')} required">
	<label for="top"> 
	   <g:message code="prodotto.top.label" default="Top" /> <span class="required-indicator">*</span>
	</label>
